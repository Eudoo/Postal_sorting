...
| | `extract_characters()` | Extraction ROI → redimensionnement 64×64 → re-binarisation |
...
Parcourt les 10 images d'entraînement (0.png à 9.png), extrait 5 chiffres par image → **50 échantillons × 11 features**. Applique la normalisation Min-Max puis calcule les centroïdes. Sauvegarde tout dans `knn_data.npz`.
...
Script principal avec sélection du mode via `--mode`. Pour chaque image de test : binarisation → segmentation → extraction features → normalisation → prédiction → annotation visuelle → affichage des métriques.
...
|---|---|---|
| 3 → 2 | k-NN + Centroïde | Formes très similaires, cavités proches |
| 7 → 1 | k-NN + Centroïde | Les deux sont allongés verticalement avec peu de cavités |
| 5 → 6 | k-NN seulement | Cavités directionnelles proches ; le centroïde le corrige grâce à la solidité |
...
|---|---|---|
| **Confusion 3↔2** | 1 erreur persistante | Q8 : Recalage en rotation (axe principal) |
| **Confusion 7↔1** | 1 erreur persistante | Q8 : Redressement de chiffres inclinés |
...
| **Chiffres qui se touchent** | Non géré actuellement | Q9 : Séparation par watershed ou morphologie |
| **Rotation des chiffres** | Chiffres inclinés mal classifiés | Q8 : Mesure de l'angle principal + rotation inverse |
